    ...
    return (
        <div className='flex justify-between items-center  bg-white py-6 px-11 max-lg:px-6 warpper max-width'>
                    ...
                    <>
                        <div className="dropdown dropdown-bottom dropdown-end dropdown-hover">
                            <div tabIndex={0} role="button" className="  flex gap-1">
                                ...
                                <Image src={IMAGES.down} alt={''} className=' hover:rotate-180 transition-transform duration-00' />
                            </div>
                            <ul tabIndex={0} className="dropdown-content z-[1] mt-4 menu  shadow bg-base-100 rounded-box w-80">
                                ...
                                {pfileItems.map((item, index) => (
                                    <li key={index} className='hover:text-primary-500 py-1 font-bold'>
                                        <Link href={item.href}>
                                            <div>{item.text}</div>
                                        </Link>
                                    </li>
                                ...
                                <div className='flex justify-center'>
                                    <button className=' text-center text-primary-500 text-xl flex justify-center  py-2' onClick={handleLogoutClick}><span className=' text-center font-bold'>Logout</span>
                                    </button>
                                </div>
                            </ul>
                        </div>
            ...
            <div className='relative lg:hidden'>
                ...
                <div
                    className={`fixed bg-white w-full h-screen overflow-hidden transition-transform flex flex-col gap-4 top-0 right-0 z-50 lg:hidden pt-4 px-6 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}
                >
                    <button
                        onClick={toggleMenu}
                    ...
                    <div className=' absolute bottom-0 w-full'>
                        <hr />
                        <button className=' text-center text-primary-500 text-xl flex justify-center pl-36 py-4'><span className=' text-center font-bold'>Logout</span>
                        </button>
                    </div>
                </div>
            </div>
